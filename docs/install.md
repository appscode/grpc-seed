...
Plow proxy server can connect to [Helm](https://github.com/kubernetes/helm) Tiller gRPC server in a number of different ways depending on the [`--connector`](/docs/reference/plow_run.md) flag.


## `incluster` Connector
Plow can proxy Tiller server running in the same Kubernetes cluster using `incluster` connector.
...
$ curl https://raw.githubusercontent.com/appscode/plow/0.2.0/hack/deploy/without-rbac.yaml \
  | kubectl apply -f -


# Install with RBAC roles
$ curl https://raw.githubusercontent.com/appscode/plow/0.2.0/hack/deploy/with-rbac.yaml \
  | kubectl apply -f -
```
...
Plow can proxy Tiller server running in a remote Kubernetes cluster using `kubeconfig` connector. In this mode, Plow open a tunnel between its own pod and Tiller server pod using Kubernetes api server. This is similar to how Helm cli connects to Tiller server today.
...
$ plow run --v=3 --connector=kubeconfig --kube-context=minikube
...
## `direct` Connector
Plow can proxy Tiller server by directly connecting to it using `direct` connector.
```console
$ plow run --v=3 --connector=direct --tiller-endpoint=http://127.0.0.1:44134
```
